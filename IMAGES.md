...
| **Aspect** | Square (1:1) |
| **Recommended size** | `w-7 h-7` (mobile) / `w-8 h-8` (desktop) → export at **64×64 px @2x** (128×128 source) |
...
| **Recommended size** | Export at **900×1200 px** (2× device pixels for retina) |
...
| **Recommended size** | **1200×900 px** source (will downscale for the card via Astro `<Image />`) |
| **Format** | WebP for product shots / UI mockups |
...
| **Aspect** | **4:3 landscape** |
| **Recommended size** | **1200×900 px** source |
| **Format** | WebP / AVIF |
...
| **Recommended** | A flat, single-colour version of the future logomark in SVG. Add `public/favicon.ico` for legacy browsers if needed. |
